...
class Packet(object):
    ...
    def __init__(self, time, idn, src="a", dst="z"):
        self.time = time
        self.id = idn
        self.src = src
        self.dst = dst
...
def packet_generator(num_packets, env, out_pipe):
...
if __name__ == '__main__':
    ...
    # prob = [queue_size for n in range(NUM_PACKETS)]
    ...
    print('Sim Total queue size   = {}' .format(queue_size))
    # list_queue_size = [queue_size for n in range(NUM_PACKETS)]
    # print(list_queue_size)
    # matplotlib.pyplot.hist(list_queue_size)
    # plt.plot(prob, label="")
    # plt.show()
